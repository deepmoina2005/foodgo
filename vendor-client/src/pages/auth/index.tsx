...
export function LoginPage() {
  const { register, handleSubmit, formState: { isSubmitting } } = useForm<AuthForm>({ defaultValues: { role: 'vendor' } });
  const dispatch = useAppDispatch();
  const navigate = useNavigate();

  const onSubmit = async (values: AuthForm) => {
    try {
      const { data } = await authApi.login({ ...values, role: 'vendor' });
  ...
  return (
    <PageShell 
      ...
      title="Vendor login" 
      description="Sign in to the vendor dashboard only."
    >
      <Card className="mx-auto mt-4 max-w-md p-8 shadow-2xl ring-1 ring-slate-200 dark:ring-white/10">
        <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
          ...
          <input type="hidden" value="vendor" {...register('role')} />
          <Button className="w-full py-4 text-base" disabled={isSubmitting}>
            {isSubmitting ? 'Signing in...' : 'Login'}
          </Button>
        </form>
        <p className="mt-8 text-center text-sm text-slate-600">
          New vendor? <Link className="font-bold text-brand-600 hover:underline" to="/signup">Create an account</Link>
        </p>
      </Card>
    </PageShell>
  );
...
export function SignupPage() {
  ...
  const onSubmit = async (values: AuthForm) => {
    try {
      const { data } = await authApi.register({ ...values, role: 'vendor' });
      if (data?.data?.user?.role !== 'vendor') {
        throw new Error('This account cannot access this dashboard.');
      }
      dispatch(setCredentials({ token: data.data.token, user: data.data.user }));
  ...
  return (
    <PageShell 
      ...
      description="Create a vendor account for the restaurant dashboard."
    >
      <Card className="mx-auto mt-4 max-w-md p-8 shadow-2xl ring-1 ring-slate-200 dark:ring-white/10">
        <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
          <input className="w-full rounded-2xl border border-slate-200 bg-white/50 px-4 py-3.5 text-sm outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 dark:border-white/10" placeholder="Restaurant / owner name" {...register('name')} />
          <input className="w-full rounded-2xl border border-slate-200 bg-white/50 px-4 py-3.5 text-sm outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 dark:border-white/10" placeholder="Email address" {...register('email')} />
