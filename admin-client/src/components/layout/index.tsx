...
const routeMap: Record<string, Array<NavItem>> = {
  vendor: [
    ...
    { to: '/vendor/documents', label: 'Documents', icon: <Menu size={18} /> },
  ...
  admin: [
    ...
    { 
      ...
      children: [
        ...
        { to: '/admin/vendor-approvals', label: 'Pending approvals' }
      ]
    },
    ...
    { 
      ...
      icon: <Users size={18} />,
      children: [
        { to: '/admin/delivery-partners', label: 'All partners' },
        { to: '/admin/delivery-approvals', label: 'Pending approvals' }
      ]
    },
  ...
  delivery_partner: [
    ...
    { to: '/delivery/kyc', label: 'KYC', icon: <Menu size={18} /> },
  ...
  return (
    <div className="space-y-1">
      <button
        ...
        className={`flex w-full items-center justify-between rounded-2xl px-4 py-3 text-sm font-semibold transition ${navClass(isActive, theme)}`}
      >
      ...
      {isOpen && (
        <div className="ml-9 mt-1 space-y-1 border-l border-[color:var(--app-border)] pl-4">
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `block rounded-xl px-3 py-2 text-sm font-medium transition ${
                  isActive 
                    ? 'text-brand-500' 
                    : theme === 'light' ? 'text-slate-500 hover:text-slate-900' : 'text-white/60 hover:text-white'
                }`
              }
            >
              {child.label}
  ...
  return (
    <div className={`flex h-full min-h-0 flex-col ${shellClass}`}>
      ...
      <nav className="mt-6 flex-1 space-y-2 overflow-y-auto pr-1">
        {navItems.map((item) => (
          <SidebarItem key={item.to} item={item} theme={theme} onNavigate={onNavigate} />
        ))}
      </nav>
  ...
  return (
    <div className="min-h-screen bg-[var(--app-bg)] text-[var(--app-text)]">
      <div className="flex min-h-screen">
        <aside className={`sticky top-0 hidden h-screen w-80 shrink-0 overflow-hidden border-r px-5 py-6 lg:flex lg:flex-col ${
          theme === 'light'
        ...
        {mobileOpen ? (
          <div className="fixed inset-0 z-40 bg-slate-950/50 lg:hidden" onClick={() => setMobileOpen(false)}>
            <aside
              className={`absolute left-0 top-0 h-full w-[85%] max-w-sm overflow-y-auto px-5 py-6 shadow-2xl ${
                theme === 'light'
